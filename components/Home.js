...
function Home() {
 ...
 /* const moviesData = [
  ...
  useEffect(()=>{
      fetch('https://mymoviz-backend-black-xi.vercel.app/movies')
  ...
  const movies = moviesData.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={data.overview} poster={`https://image.tmdb.org/t/p/w500${data.poster_path}`} voteAverage={data.voteAverage} voteCount={data.vote_count} />;
  });
